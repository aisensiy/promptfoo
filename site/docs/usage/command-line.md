...
| `PROMPTFOO_SERVER_HOST`                       | Host address for the local web UI server. Defaults to loopback for local CLI use; set to `0.0.0.0` for trusted container or remote deployments that must accept non-local connections.                                                                                            | `localhost`                   |
